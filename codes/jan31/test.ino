...
QTRSensorsRC qtrrc((unsigned char[]) { A8, A9, A10, A11, A12, A13, A14, A15}, // six middle sensors used to follow a line
  NUM_SENSORS, TIMEOUT, EMITTER_PIN);

unsigned int sensorValues[NUM_SENSORS]; // array of the six middle sensors

LiquidCrystal lcd(22, 24, 10, 11, 12, 13); // lcd object to display pid values

QTRSensorsRC qtrrcA((unsigned char[]) { A1, A2 }, // two outer sensors used to detect cross-sections (not used in tuning)
...
int const qtrsetpoint = 2500;    // constant setpoint for PID
int const sharpsetpoint = 260;
int speedRight;    // variable for the speed of right motor
int speedLeft;    // variable for the speed of left motor
int max_speed = 255;    // defined maximum speed
int error = 0;    // initialized error for PID
float Ki = 0, Kd = 0.2, Kp = 0.05;    // define values for calculatin ERROR on PID
float wKi = 0, wKd = 0, wKp = 0.05;  // define values for calculating error in wall follow

int speedLeftBase = 100;
int speedRightBase = 100;  // base speeds of left and right motors
int speedLeftBaseWall = 60;
int speedRightBaseWall = 60;  //base speeds of motors wall follow

unsigned long previousMillis = 0;
unsigned long currentMillis = 0;
const long decisionTimeOut = 1200;
bool initialDrive, pidFollowTrue, decisionBool, deliveredState, phaseOne, phaseTwo, phaseThree, timeSetReverse;
float increment[] = { 0.1, 0.01, 0.001, -0.1, -0.01, -0.001 }; // array housing increment values
char mapChar[] = {'R', 'F', 'R', 'R', 'L', 'L', 'R', 'R', 'R', 'L', 'R', 'R', 'F', 'R', 'F', 'F', 'R'};
byte mapCount = 0;
bool mapDelivery[] = {true, false, true, true, false, false, true, true, false, false, true, true, false, true, false, true};
...
void loop()
{
  while (pidFollowTrue && !initialDrive)
    pid();
  while (initialDrive && !pidFollowTrue)
    initD();
}
...
void initD()
{
  ...
  if(sensorValues[2] > 700 || sensorValues[3] > 700)
  {
    analogWrite(ENA, speedRightBase+60);
...
void decision()
{
  if(!mapDelivery[mapCount] || deliveredState)
  {
